...
const IndexPage = () => {
  ...
  const uploadStreamingData = async () => {
    ...
    for (let i = 0; i < selectedFiles.length; i++) {
      formData.append('streaming_data', selectedFiles[i]);
    }

    formData.append('start_date', startDate.toISOString().slice(0, 16).replace('T', ' '));
    formData.append('end_date', endDate.toISOString().slice(0, 16).replace('T', ' '));
  ...
  return (
    <div className="container text-center my-5">
      ...
      <div className="my-4">
        ...
        <a href="http://127.0.0.1:5000/login" className="btn btn-success btn-lg mb-4">
          Login to Spotify
        </a>
